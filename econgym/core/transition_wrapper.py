...
class TransitionConfig:
    ...
    num_periods: int = 100  # Number of periods to simulate
    num_agents: int = 1000  # Number of agents to track
    burn_in: int = 50      # Number of periods to burn in before transition
    transition_period: int = 50  # Period at which the transition occurs
...
class TransitionWrapper(EconEnv):
    """
    ...
    def step(self, action):
        ...
        if self.config.compute_moments:
            moments = self._compute_moments(next_state, action, reward)
            self.transition_data['moments'].append(moments)
    ...
    def simulate_transition(self, 
        ...
        for _ in range(self.config.burn_in):
            ...
            initial_state, _, _, _ = self.env.step(action)
