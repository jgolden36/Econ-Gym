from aiyagari_transition import AiyagariTransition
...
def main():
    ...
    # Define final parameters (low capital tax state)
    final_params = {
        'beta': 0.96,
        'risk_aversion': 2.0,
        'income_persistence': 0.9,
        'labor_tax': 0.2,
        'capital_tax': 0.2,  # Final capital tax rate (20%)
        'alpha': 0.36,
        'delta': 0.08,
        'num_agent_types': 2,
        'X': 1.0
    ...
    try:
        ...
        sys.stdout.flush()
        transition.plot_transition(analysis, save_path="aiyagari_transition_results.png")

        print("\nAnalysis complete! Results saved to 'aiyagari_transition_results.png'", flush=True)
        sys.stdout.flush()
